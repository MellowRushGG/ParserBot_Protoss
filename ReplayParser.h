...
struct P_BuildOrderEntry {
    ...
    P_BuildOrderEntry(uint32_t pStep, UNIT_TYPEID pType) {
        step = pStep;
        unitType = pType;
        isUpgrade = false;
    }

    P_BuildOrderEntry(uint32_t pStep, UPGRADE_ID pType) {
        step = pStep;
        upgradeId = pType;
        isUpgrade = true;
    }
...
class P_ReplayParser : public ReplayObserver {
public:
    P_ReplayParser(int id);
    ~P_ReplayParser();
...
private:
    ...
    std::string mCurrentReplayName;
    std::unordered_set<UNIT_TYPEID> mUnitsOrdersImInterestedIn;
    std::vector<P_BuildOrderEntry> mBuildOrdersFromActions;
    std::vector<P_BuildOrderEntry> mBuildOrdersFromOnCreated;
    std::unordered_set<Tag> mSeenUnits;
    float mLastFeatures[int(P_SECTORS::MAX)];
    float mCurrentFeatures[int(P_SECTORS::MAX)]{};
    int mProcessedSteps = 0;
};